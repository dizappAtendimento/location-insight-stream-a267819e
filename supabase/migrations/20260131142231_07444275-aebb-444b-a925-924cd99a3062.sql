...
-- Fix: Update create_disparo function to create separate detail records for each media in a message
-- When a message has multiple medias, each media becomes a separate dispatch detail
...
DECLARE
  ...
  -- Modo de mensagem (variation ou sequence)
  v_message_mode text := COALESCE(p_payload #>> '{settings,messageMode}', 'variation');
  v_sequence_interval int := COALESCE((p_payload #>> '{settings,sequenceInterval}')::int, 5);
  ...
  v_msg_idx int;
  v_contact_ts timestamptz;
  v_variations jsonb;
  v_variation_count int;
  v_selected_idx int;
  v_all_texts text[];
  v_medias_array jsonb;
  v_media_count int;
  v_media_idx int;
  v_single_media jsonb;
BEGIN
  -- Check plan limits
  ...
  IF v_limit_plan IS NOT NULL THEN
    ...
    -- No modo sequência, multiplicar pelo número de mensagens
    IF v_message_mode = 'sequence' THEN
      v_to_send := v_to_send * v_msg_count;
    END IF;
  ...
  LOOP
    ...
    -- Guardar timestamp inicial para este contato
    v_contact_ts := v_cur_ts;

    IF v_message_mode = 'sequence' THEN
      -- MODO SEQUÊNCIA: Enviar TODAS as mensagens para este contato
      FOR v_msg_idx IN 1..v_msg_count LOOP
        v_msg_obj := v_messages[v_msg_idx];
        
        -- Obter texto original e variações
        v_template := v_msg_obj->>'text';
        v_variations := v_msg_obj->'variations';
        
        -- Se tem variações, escolher aleatoriamente entre o texto original e as variações
        IF v_variations IS NOT NULL AND jsonb_array_length(v_variations) > 0 THEN
          v_all_texts := ARRAY[COALESCE(v_template, '')];
          FOR i IN 0..jsonb_array_length(v_variations)-1 LOOP
            v_all_texts := v_all_texts || (v_variations->>i);
          END LOOP;
          v_selected_idx := floor(random() * array_length(v_all_texts, 1))::int + 1;
          v_template := v_all_texts[v_selected_idx];
        END IF;
        
        -- Get medias array
        v_medias_array := COALESCE(v_msg_obj->'medias', '[]'::jsonb);
        v_media_count := jsonb_array_length(v_medias_array);
        ...
        v_rendered := public.f_render_message(v_template, rec_cont.nome, rec_cont.atributos, v_contact_ts);

        IF v_media_count > 0 THEN
          -- Multiple medias: create a separate detail for EACH media
          FOR v_media_idx IN 0..v_media_count-1 LOOP
            v_single_media := v_medias_array->v_media_idx;
            v_media_payload := jsonb_build_object('media', v_single_media);
            
            -- Only include text in the LAST media of this message
            INSERT INTO public."SAAS_Detalhes_Disparos"
              ("idDisparo","idContato","Mensagem","idConexao","dataEnvio","Status","Payload","FakeCall")
            VALUES (
              v_disparo_id, 
              rec_cont.id, 
              CASE WHEN v_media_idx = v_media_count - 1 THEN v_rendered ELSE NULL END, -- text only on last media
              v_conn_id, 
              v_contact_ts, 
              'pending', 
              v_media_payload, 
            ...
            IF v_total = 0 THEN v_first_ts := v_contact_ts; END IF;
            v_total := v_total + 1;
            
            -- Small interval between medias (1 second)
            IF v_media_idx < v_media_count - 1 THEN
              v_contact_ts := v_contact_ts + interval '1 second';
            END IF;
          END LOOP;
        ELSE
          -- No medias, just text
          INSERT INTO public."SAAS_Detalhes_Disparos"
            ("idDisparo","idContato","Mensagem","idConexao","dataEnvio","Status","Payload","FakeCall")
          VALUES (v_disparo_id, rec_cont.id, v_rendered, v_conn_id, v_contact_ts, 'pending', NULL, v_fake_call);

          IF v_total = 0 THEN v_first_ts := v_contact_ts; END IF;
          v_total := v_total + 1;
        END IF;

        -- Adicionar intervalo entre mensagens da sequência (exceto última)
        IF v_msg_idx < v_msg_count THEN
          v_contact_ts := v_contact_ts + make_interval(secs => v_sequence_interval);
        END IF;
      END LOOP;
    ...
    ELSE
      -- MODO VARIAÇÃO (padrão): Uma mensagem por contato, rotacionando
      v_msg_obj := v_messages[v_idx_msg];
      v_idx_msg := (v_idx_msg % v_msg_count) + 1;
      v_template := v_msg_obj->>'text';
      
      -- Se tem variações no modo variação, também escolher aleatoriamente
      v_variations := v_msg_obj->'variations';
      IF v_variations IS NOT NULL AND jsonb_array_length(v_variations) > 0 THEN
        v_all_texts := ARRAY[COALESCE(v_template, '')];
        FOR i IN 0..jsonb_array_length(v_variations)-1 LOOP
      ...
      IF v_media_count > 0 THEN
        ...
        FOR v_media_idx IN 0..v_media_count-1 LOOP
          ...
          VALUES (
            v_disparo_id, 
            rec_cont.id, 
            CASE WHEN v_media_idx = v_media_count - 1 THEN v_rendered ELSE NULL END,
            v_conn_id, 
            v_cur_ts, 
            'pending', 
            v_media_payload, 
          ...
          -- Small interval between medias (1 second)
          IF v_media_idx < v_media_count - 1 THEN
            v_cur_ts := v_cur_ts + interval '1 second';
          END IF;
        END LOOP;
      ELSE
        -- No medias, just text
        INSERT INTO public."SAAS_Detalhes_Disparos"
          ("idDisparo","idContato","Mensagem","idConexao","dataEnvio","Status","Payload","FakeCall")
        VALUES (v_disparo_id, rec_cont.id, v_rendered, v_conn_id, v_cur_ts, 'pending', NULL, v_fake_call);
