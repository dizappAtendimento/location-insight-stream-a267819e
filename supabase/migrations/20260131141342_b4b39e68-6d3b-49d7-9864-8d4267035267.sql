...
-- Fix: Update create_disparo function to store ALL medias, not just the first one
    ...
    IF v_message_mode = 'sequence' THEN
      ...
      FOR v_msg_idx IN 1..v_msg_count LOOP
        ...
        IF v_variations IS NOT NULL AND jsonb_array_length(v_variations) > 0 THEN
          -- Criar array com texto original + variações
        ...
        -- FIX: Extract ALL medias, not just the first one
        v_media_payload := NULL;
        IF v_msg_obj->'medias' IS NOT NULL AND jsonb_array_length(COALESCE(v_msg_obj->'medias', '[]'::jsonb)) > 0 THEN
          -- Store the full medias array
          v_media_payload := jsonb_build_object('medias', v_msg_obj->'medias');
        ELSIF v_msg_obj->'media' IS NOT NULL AND v_msg_obj->'media'->>'link' IS NOT NULL THEN
          -- Legacy: single media as array for consistency
          v_media_payload := jsonb_build_object('medias', jsonb_build_array(v_msg_obj->'media'));
        END IF;
        ...
        VALUES (v_disparo_id, rec_cont.id, v_rendered, v_conn_id, v_contact_ts, 'pending', v_media_payload, v_fake_call);
